<script lang="ts">
	...
	let compactMode = $state(
		browser
			? (localStorage.getItem('compactMode') ?? String(initialCompactMode)) === 'true'
			: initialCompactMode
	);
	let query = $state('');
	let sortBy = $state<'recent' | 'downloads'>(
		browser
			? ((localStorage.getItem('sortBy') as 'recent' | 'downloads') ?? 'downloads')
			: 'downloads'
	);
	...
	{:else}
		{#each filteredResources as resource (resource.id)}
			<div animate:flip={{ duration: 300 }} class="border-b border-zinc-200 last:border-b-0">
				<Dialog.Root>
					<Dialog.Trigger class="p-2 hover:bg-zinc-100 w-full text-left cursor-pointer">
						{#if compactMode}
